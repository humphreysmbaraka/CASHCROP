             ...
             <>
              ...
              {sampleorders?.map((order, i) => (
                ...
                >
                  ...
                  <Pressable onPress={()=>{navigation.navigate('vieworder' , {order:order._id ,viewingsale:(activeTab=='Sales') , viewingpurchase:(activeTab=='Purchases') , viewingpendingpayment:(activeTab=='Pending payments') , viewingsettled:(activeTab=='Settled') , viewingbuyerorder:(activeTab=='All')  , setpurchases , setsales , setpendingpays , setsettled ,setsampleorders,  setbuyerorders})}} width={'99%'} p={'2px'}  >
                     <HStack width={'100%'} p={'3px'}   >
                      ...
                      <VStack p={'4px'} >
                        ...
                        <Button   mr={'10px'}   p={'3px'} colorScheme={order?.status=='NEW'?'green':order?.status=='PENDING'?'purple':order?.status=='CANCELLED'?'red':order?.status=='DELIVERED'?'gray':order?.status=='COMPLETED'?'green':order?.status=='REVERSED'?'red':order?.status=='WAITING FOR REFUND'?'orange':order?.status=='REFUNDED'?'orange':'purple'}  >{order?.status}</Button>
                      </VStack>
                     </HStack>
                  </Pressable>
                </Box>
