...
export default function CreateShop({navigation}) {
  ...
  const [paymentmethod , setpaymentmethod] = useState(null); // FOR PAYING SHOP RENT (mpesa/card)
  const [payaccount1 , setpayaccount1] = useState(null); //MPESA NUMBER OR BANK ACCOUNT NUMBER
  const [payaccount2 , setpayaccount2] = useState(null);
  const [disbursementmethod , setdisbursementmethod] = useState(null); // FOR RECEIVING PAYMENT
  const [disburseaccount1 , setdisburseaccount1] = useState(null); // MPESA NUMBER OR BANK ACCOUNT NUMBER
  ...
  const [bankaccountname1 , setbankaccountname1] = useState(null); // for paying rent
  const [bankaccountname2 , setbankaccountname2] = useState(null);
  
  const [disburseaccountname1 , setdisburseaccountname1] = useState(null); // for receiving payments
